...
public class PanelSearchProduct extends JPanel {
	...
	public void cargarResultados() {
		...
		ArrayList<Producto> listaProductos = daoProduct
				.consultarOptimizado(getFieldSearch().getText());
		...
		for (int i = 0; i < listaProductos.size(); i++) {
			...
			mostrar = daoVarios.consultarVariosPorCategoriaNivel2(
					"Tipo de Elemento", Integer.parseInt(listaProductos.get(i)
							.getIdCategoriaProducto()))
					+ " "
					+ daoVarios.consultarVariosPorCategoriaNivel2(
							"Marca Elemento", Integer.parseInt(listaProductos
									.get(i).getIdMarcaProducto()))
					+ " "
			...
			getModelListResult().addElement(
					new Datos(listaProductos.get(i).getIdProducto(), mostrar));
			mostrar = "";
		}
	}
