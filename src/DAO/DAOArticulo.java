...
public class DAOArticulo {
	...
	public Articulo consultar(String idArticulo) {
		...
		try {
			ResultSet rs = m_DAOConnectionLogin.executeQuery("select * from articulo where idarticulo = '" + idArticulo + "'");
			while (rs.next()) {
				articuloResultado.setIdArticulo(Integer.parseInt(rs.getString("idarticulo")));
				articuloResultado.setIdProductoArticulo(Integer.parseInt(rs.getString("idproductoarticulo")));
				articuloResultado.setNumeroSerie(rs.getString("numeroserie"));
				articuloResultado.setEstado(Integer.parseInt(rs.getString("estado")));
				articuloResultado.setIdProveedor(rs.getString("idproveedor"));
				articuloResultado.setCostoArticulo(Integer.parseInt(rs.getString("costoarticulo")));
				articuloResultado.setFechaIngreso(rs.getString("fechaingreso"));
				articuloResultado.setIdDocumentoSoporte(rs.getString("iddocumentosoporte"));
	...
	public boolean verificarProductoRegistrado(String idProductoArticulo) {
		try {
			ResultSet rs = m_DAOConnectionLogin.executeQuery("select * from articulo where idProductoArticulo = '" + idProductoArticulo + "'");
			while (rs.next()) {
				return true;
	...
	public ArrayList<Articulo> buscarArticulosPorProducto(String idProductoArticulo){
	...
	public ArrayList<Articulo> buscarArticulosPorProductoDisponibles(String idProductoArticulo){
		ArrayList<Articulo> temporal = new ArrayList<Articulo>();
		
		try {
			ResultSet rs = m_DAOConnectionLogin.executeQuery("select * from articulo where idProductoArticulo = '" + idProductoArticulo + "' and estado='0'");
			while (rs.next()) {
				Articulo articuloResultado = new Articulo();
				articuloResultado.setIdArticulo(Integer.parseInt(rs.getString("idarticulo")));
				articuloResultado.setIdProductoArticulo(Integer.parseInt(rs.getString("idproductoarticulo")));
	...
	public int editar(Articulo articulo) {
		...
		String sql = "Update articulo Set idProductoArticulo='" + articulo.getIdProductoArticulo() + "',"
				+ "numeroSerie='" + articulo.getNumeroSerie() + "',"
				+ "estado=" + articulo.getEstado() + ","
				+ "idProveedor='" + articulo.getIdProveedor() + "',"
				+ "costoArticulo=" + articulo.getCostoArticulo() + ","
				+ "fechaIngreso=" + articulo.getFechaIngreso() + ","	
				+ "idsitio=" + articulo.getIdSitio() + ","	
				+ "idDocumentoSoporte='" + articulo.getIdDocumentoSoporte()+ "'";
	...
	public int insert(Articulo articulo) {
		...
		sql = "INSERT INTO articulo (idProductoArticulo ," +
				...
				"idDocumentoSoporte,idsitio)" +
				"values ('"+articulo.getIdProductoArticulo()+"', " +
				"'"+articulo.getNumeroSerie()+"', " +
				"'"+articulo.getEstado()+"', '"+articulo.getIdProveedor()+"', " +
				"'"+articulo.getCostoArticulo()+"', " +
				" '"+articulo.getIdDocumentoSoporte()+"','"+articulo.getIdSitio()+"')";
